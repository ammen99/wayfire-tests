...
class WTest(wt.WayfireTest):
    ...
    def _run(self):
        ...
        self._click_and_drag((50, 50), (250, 50))
        self.wait_for_clients(2)
        ...
        self._click_and_drag((450, 50), (440, 40))
        self.wait_for_clients(2)
        if not gtk2.expect_line("drag-begin"):
            return wt.Status.WRONG, 'gtk2 did not start second drag: ' + gtk2.last_line
        if not gtk2.expect_line("drag-drop"):
            return wt.Status.WRONG, 'gtk2 did not receive second drag drop: ' + gtk2.last_line
        if not gtk2.expect_line("drag-end"):
            return wt.Status.WRONG, 'gtk2 did not end second drag: ' + gtk2.last_line
        if not gtk1.expect_none() or not gtk2.expect_none():
            return wt.Status.WRONG, 'gtk1/gtk2 have extra output: ' + gtk1.last_line + '$' + gtk2.last_line
